 ...
 * 叠字布局与 [PhotoOverlayComposer]（AR 截图）一致：居中 480×640 比例内缩框、框内左对齐多行、绿色与 sp 映射相同。
...
object ArVideoMedia3OverlayExporter {
    ...
    private class TimelinePhotoStyleBitmapOverlay(
        ...
        override fun getBitmap(presentationTimeUs: Long): Bitmap {
            ...
            val raw = segmentsMs.firstOrNull { tMs >= it.first && tMs < it.second }?.third.orEmpty()
            if (raw.isEmpty()) return blankFrame
            synchronized(lock) {
    ...
    fun burnInTimeline(
        ...
        val overlay = TimelinePhotoStyleBitmapOverlay(segmentsMs, vw, vh, textSizeSp)
        ...
        val transformerContext = context.applicationContext

        val mainHandler = Handler(Looper.getMainLooper())
        Log.d(TAG, "[burnIn] schedule Transformer on main thread (caller=${Thread.currentThread().name})")
        mainHandler.post {
            try {
                val transformer = Transformer.Builder(transformerContext).build()
